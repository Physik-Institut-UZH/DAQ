//-------------------------------------------------------------------
//  graph.cpp   --   miniDAX: DAQ for XENON
// 
		...
		TH1D *g, var_graphics& graphics)
{
  ...
  tex->SetLineWidth(2);
  tex->Draw();
  tex = new TLatex(0.1702128,0.8053528,"mini");
  tex->SetTextColor(4);
  tex->SetTextFont(72);
        ...
        if ((buff[pnt]>>28)==0x8) { // good data
	     ...
	     pnt++; wavecnt+=2; cnt++;
          ...
          if (buff[pnt]!=0xFFFFFFFF) {
            ...
            pnt++; cnt++;
          } else { errormsg(0,(char*)"skip"); return -1; }
        }
      } // end while(cnt...)
    } // end for-loop
  } // end Check Header 
  else { errormsg(0,(char*)"Header"); return -1; }
...
//      g->GetXaxis()->SetRangeUser(0,adc.NbSamplesEvt-1);
      g->GetXaxis()->SetRangeUser(0,adc.EventLength-1);
      break;
...
int graph_checkkey(var_graphics& graphics, digitizer& adc, char c, TH1D *g, char *OutFileName)
{
  ...
  if (c == '1' || c == '2' || c == '0' || c == 'u' || c == 'd' || c == 'U' || c == 'D') {
    switch (c) {
      case '0': graphics.y_display=0; 	
		col((char*)"green"); printf("y-axis: standard display mode\n"); col((char*)"blue"); // DM
	break;
      case '1': graphics.y_display=1; 	
		col((char*)"green"); printf("y-axis: manual display mode\n"); col((char*)"blue"); // DM
	break;
      case '2': graphics.y_display=2; 	
		col((char*)"green"); printf("y-axis: automatic display mode\n"); col((char*)"blue");
	break;
  ...
  if (c == '3' || c == '4' || c == '5' || c == 'l' || c == 'r' || c == 'L' || c == 'R' || 'o') {
    switch (c) {
      case '3': graphics.x_display=0; 	
		col((char*)"green"); printf("x-axis: standard display mode\n"); col((char*)"blue");
	break;
      case '4': graphics.x_display=1; 	
		col((char*)"green"); printf("x-axis: manual display mode\n"); col((char*)"blue");
	break;
      case '5': graphics.x_display=2; 	
		col((char*)"green"); printf("x-axis: automatic display mode\n"); col((char*)"blue");
	break;
  ...
  if (c == 's' || c == 'S') {
    ...
    if (f==NULL) errormsg(0,(char*)":::: WARNING: cannot store waveform! ::::");
    else {
      col((char*)"green"); printf("store waveform to file %s\n",str); col((char*)"blue");
      f->cd();
      g->Write();
      f->Close();
      counter++;
  ...
  if (c == 'b' || c == 'B') {
    ...
    col((char*)"green"); printf("Waveform PMT %d: mean = %.1lf, RMS = %.6lf\n",
