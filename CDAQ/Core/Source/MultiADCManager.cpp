...
#include <iostream>
#include <string>
#include "MultiADCManager.h"
#include "global.h"


MultiADCManager::MultiADCManager()
{
	...
	printf("\n");
	printf("%s*****************************************************************\n",KGRN);
	printf("%s             Multi ADC DAQ --  Data Aquistion Program	    	   \n",KGRN); 
...
int MultiADCManager::ProcessInput( int argc, char *argv[], char *envp[])
{
...
int MultiADCManager::StartAquistion(){
...
int MultiADCManager::StopAquistion(){
...
int MultiADCManager::ShowStatus(int status ){
		if(status==1)
			m_events++;
...
int MultiADCManager::ApplyXMLFile(){
	int temp;  
	char txt[100];
	const char *xstr;
	txt[0]='\0';
...
int MultiADCManager::xml_storefile()
{
  char cmnd[300];
