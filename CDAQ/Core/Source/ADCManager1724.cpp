...
ADCManager1724::ADCManager1724():ADCManager(0)
{
...
bool ADCManager1724::OpenDigitizer(){
  ...
  SetPostTriggerSize(m_handle,m_PostTrigger);
  ...
  for (int ch = 0; ch < m_nbCh; ch++) {
    if (m_EnableMask & (1<<ch)) {
      ...
      CAEN_DGTZ_TriggerPolarity_t *val;
      m_Ret = CAEN_DGTZ_GetTriggerPolarity(m_handle,ch,val);
      if(*val == CAEN_DGTZ_TriggerOnRisingEdge) cout<<"Positive trigger for channel "<<ch<<endl;
      else cout<<"Negative trigger for channel "<<ch<<endl;
      int check = 0;
    }
  }
  //*/
...
CAEN_DGTZ_ErrorCode ADCManager1724::OrProgrammer(){
  int m_ret=0;
...
int ADCManager1724::CheckEventBuffer(int eventCounter){
  //Do something
...
int ADCManager1724::ApplyXMLFile(){
  ...
  xstr=xNode.getChildNode("custom_size").getText();
    ...
    m_PostTrigger = m_RecordLength*(m_PostTrigger/100.)/2;
