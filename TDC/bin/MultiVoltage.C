...
void MultiVoltage(){
	...
	if (inputFolder.good())
   	{
    	...
    	{	
			...
			for(int ch=0; ch<nChanels; ch++){
				...
				graphs[ch][counter]->GetYaxis()->SetTitle("DC Rate [Hz]");
	...
	for(int g=0;g<nChanels;g++){
		...
		std::stringstream ss;
		ss << "c" << g;
		c1[g]=  new TCanvas(ss.str().c_str(),"",200,10, 1920, 1080);
		c1[g]->SetLogy();
		mg[g]->Draw("APL");
		mg[g]->SetTitle("DarkCount vs. Threshold for CH2 at T= 170 K");
	...
	std::vector<double> HeightError;
	std::vector<double> SigmaError;
	inputFileName.open("height_300k.txt", ios::in);	
	if (inputFileName.good()){
		inputFileName.seekg(0L, ios::beg);
			while (! inputFileName.eof()){
    				inputFileName >> tmp1 >> tmp2 >> tmp3 >>tmp4 >> tmp5;	//For every colum
				if( inputFileName.eof() ) break;
				Voltage.push_back(tmp1);
				Height.push_back(tmp2);
				Sigma.push_back(tmp3);
				HeightError.push_back(tmp4);
				SigmaError.push_back(tmp5);
			}
	}
	...
	for(int i=0;i<Voltage.size();i++){
		...
		std::cout << Voltage[i] << "	" << Height[i] << "	+-	" << HeightError[i] << "	"  << Sigma[i] << "	+-	" <<  SigmaError[i] << std::endl;
	...
	TGraphErrors* dc = new TGraphErrors();
	TGraphErrors* ct = new TGraphErrors();

	for(int i=0;i<Voltage.size();i++){
		double dc05 = (Height[i]+1)*0.5;
		double dc15 = (Height[i]+1)*1.5;
		...
		std::cout << "DC 0.5PE:	" << graphs[0][i]->Eval(dc05);
		std::cout << std::endl;
		std::cout << "DC 1.5PE:	" << graphs[0][i]->Eval(dc15);
		std::cout << std::endl;


		double dc005 = graphs[0][i]->Eval(dc05);
		double dc015 = graphs[0][i]->Eval(dc15);
		std::cout << "CT:	" << (dc015/dc005)*100 ;
		std::cout << "DC05:	" << dc005 << "DC15:	" << dc015 << std::endl;
		std::cout << std::endl;
		...
		ct->SetMarkerColor(4);
		ct->SetMarkerSize(3);
		ct->SetMarkerStyle(31);

		dc->SetPoint(i,Voltage[i],dc005);
		//plots->SetPointError(i,0.01,0);	

		ct->SetPoint(i,Voltage[i],(dc015/dc005)*100 );
