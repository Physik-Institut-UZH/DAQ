...
int ADCManager::RegisterReading(){
	printf(KYEL);
...
int ADCManager::ApplyXMLFile(){
	...
	for(int i=0;i<8;i++){
     ...
     if (xstr) {
        ...
        if(temp!=0){
			m_hex=m_hex+pow(2,i);
			adc_writereg(ChannelTreshold+(i*0x0100),temp);
			//Input Range 0-2V!
			adc_writereg(ChannelRange+(i*0x0100),0);
		}
	...
	adc_writereg(CustomWindow,temp);
    ...
    if (temp==0) {
		...
		m_hex=0xC0000000;
		adc_writereg(FrontPanelTriggerOut,m_hex);
	...
	if (xstr) {
		...
		m_hex=(int)(atoi(txt));
		if(temp==1){
			adc_writereg(FrontPanelIOData,m_hex);
		}
		else{
			adc_writereg(FrontPanelIOData,m_hex);
		}
...
int ADCManager::Checkkeyboard(char c){
	...
	if (c == 'w') {
		...
		for(int i=0;i<8;i++){
			...
			adc_writereg(ChannelTreshold+(i*0x0100),channelTresh[i]);
		}
		this->Enable();
	}
